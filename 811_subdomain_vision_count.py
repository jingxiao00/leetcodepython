...
"""Learned
1. A count is a dict that with default value as 0, can be import by: colleciton.Counter()
...
class Solution(object):
    def subdomainVisits(self, cpdomains):
        ...
        for items in cpdomains:
            count, cpdomain = items.split(' ')
            ls = cpdomain.split('.')
            if len(ls) is 2:
                domains = [ls[-1], '.'.join(ls)]
            elif len(ls) is 3:
                domains = [ls[-1], '.'.join(ls[-2:]), '.'.join(ls)]
